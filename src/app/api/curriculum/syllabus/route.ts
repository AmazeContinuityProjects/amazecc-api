/**
 * @openapi
 * /api/curriculum/syllabus:
 *   post:
 *     tags:
 *       - Curriculum
 *     summary: Auto-generated POST endpoint for /api/curriculum/syllabus
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(req: Request) {
  try {
    const { cookies, authorizedID, csrf, courseCode } = await req.json().catch(() => ({}));
    const cookieHeader = Array.isArray(cookies) ? cookies.join("; ") : cookies;
    if (!csrf || !authorizedID || !courseCode) {
      return NextResponse.json({ error: "Missing csrf, authorizedID, or courseCode" }, { status: 400 });
    }
    ...
    const downloadRes = await fetch(
      "https://vtopcc.vit.ac.in/vtop/courseSyllabusDownload1",
      {
        method: "POST",
        headers: {
          Cookie: cookieHeader,
    ...
    if (!downloadRes.ok) {
      ...
      console.error("syllabus fail:", downloadRes.status, contentType, snippet.substring(0, 200));
