/**
 * @openapi
 * /api/mooc-registration:
 *   post:
 *     tags:
 *       - Mooc Registration
 *     summary: Auto-generated POST endpoint for /api/mooc-registration
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(req: Request) {
  try {
    ...
    const resp = await client.post(
      "/vtop/academics/mooc/studentRegistration",
      new URLSearchParams({
    ...
    console.error("mooc-registration error:", (err instanceof Error ? err.message : String(err)));
