...
/**
 * @openapi
 * /api/hostel-counselling:
 *   post:
 *     tags:
 *       - Hostel Counselling OTP
 *     summary: POST endpoint for /api/hostel-counselling
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(req: Request) {
  try {
    ...
    const resp = await client.post(
      "/vtop/examinations/technoCrendentials",
      new URLSearchParams({
        verifyMenu: "true",
        authorizedID,
        _csrf: csrf,
    ...
    console.error("hostel-counselling error:", (err instanceof Error ? err.message : String(err)));
