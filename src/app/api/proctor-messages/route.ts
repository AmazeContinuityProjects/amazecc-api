/**
 * @openapi
 * /api/proctor-messages:
 *   post:
 *     tags:
 *       - Proctor Messages
 *     summary: Auto-generated POST endpoint for /api/proctor-messages
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(req: Request) {
  try {
    ...
    const resp = await client.post(
      "/vtop/proctor/viewMessagesSendByProctor",
      new URLSearchParams({
    ...
    console.error("proctor-messages error:", (err instanceof Error ? err.message : String(err)));
