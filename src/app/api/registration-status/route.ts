 ...
 *     tags:
 *       - Registration Status
 *     summary: Auto-generated POST endpoint for /api/registration-status
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
  ...
  try {
    ...
    if (semesterId) {
      const dataResp = await client.post(
        "/vtop/research/processRegistrationViewtoScholar",
        new URLSearchParams({
          authorizedID, x: new Date().toUTCString(),
          semid: semesterId, _csrf: csrf,
        }).toString(),
        { headers }
      );
    ...
    const resp = await client.post(
      "/vtop/research/CourseworkRegistrationViewtoScholar",
      new URLSearchParams({
    ...
    console.error("registration-status error:", (err instanceof Error ? err.message : String(err)));
