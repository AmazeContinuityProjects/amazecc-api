 ...
 *     tags:
 *       - Mooc Upload
 *     summary: Auto-generated POST endpoint for /api/mooc-upload
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
  ...
  try {
    ...
    if (semesterId) {
      const dataResp = await client.post(
        "/vtop/examinations/gotToMoocCourseOnChange",
        new URLSearchParams({
    ...
    const pageResp = await client.post(
      "/vtop/examinations/gotToMoocCourseInitial",
      new URLSearchParams({
    ...
    console.error("mooc-upload error:", (err instanceof Error ? err.message : String(err)));
