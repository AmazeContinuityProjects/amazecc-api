/**
 * @openapi
 * /api/wishlist-registration:
 *   post:
 *     tags:
 *       - Wishlist Registration
 *     summary: Auto-generated POST endpoint for /api/wishlist-registration
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
...
export async function POST(req: Request) {
  try {
    ...
    const initResp = await client.post(
      "/vtop/academics/common/WishlistStudent",
      new URLSearchParams({
    ...
    for (const opt of options) {
      try {
        const dataResp = await client.post(
          "/vtop/academics/processViewWishList",
          new URLSearchParams({
