...
export async function POST(req: Request) {
  try {
    ...
    const INIT_URL = "/vtop/examinations/arrearRegistration/LoadRegularArrearViewPage";
    const DATA_URL = "/vtop/examinations/arrearRegistration/viewRARArrearBySemesterWise";
    ...
    let semFieldName = "selExamTypeId";
    $("select").each((_, el) => {
    ...
    for (const opt of options) {
      try {
        const dataResp = await client.post(DATA_URL, new URLSearchParams({ authorizedID, x: new Date().toUTCString(), [semFieldName]: opt.value, _csrf: csrf }).toString(), { headers });
