...
import { parseCompreExam } from "@/lib/parsers/compre-exam";
...
export async function POST(req: Request) {
  try {
    ...
    const data = parseCompreExam(resp.data);
    return NextResponse.json({ success: true, ...data });
  } catch (err: any) {
    console.error("compre-exam error:", err.message);
